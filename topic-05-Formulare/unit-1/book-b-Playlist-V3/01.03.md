...
Hier wird versucht das Attribut `title` aus dem Objekt `body` des Requests auszulesen. Dieses Objekt `body`  (und dessen Attribut `title`) befüllt Express mit den übermittelten Formulardaten aus dem `POST`-Request. Damit dies wie erwartet funktioniert muss dazu `app.js` durch die folgenden Zeilen ergänzt werden:
...
```
Achten Sie darauf, dass diese beiden Zeilen Zugriff auf das Objekt `app` benötigen. D.h. die Zeilen müssen unter der Zeile `const app = express();` eingefügt werden.
