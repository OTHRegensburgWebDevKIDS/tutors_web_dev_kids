...
let songs = songStore.getAllSongs();
```

Ergänzen Sie den Code so, dass die folgende Ausgabe erzeugt wird, wenn Sie das Programm mit *Node.js* ausführen. Der `songStore` muss dazu alle `songs`returnen und Sie müssen die `songs` außerhalb des `songStore` auf dem Terminal ausgeben.:
...
`songs.js` (noch unvollständig, Lösung von oben fehlt):
